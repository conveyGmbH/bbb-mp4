...
ffmpeg -y \
       ...
       -c:v libvpx -vf 'scale=1280:-1' -crf 28 -preset slow -threads 1 -pass 1 -an -f null /dev/null
echo "encoding $1-1280.webm started pass 2"
ffmpeg -y \
       -i /usr/src/app/download/$1.mp4 \
       -c:v libvpx -vf 'scale=1280:-1' -crf 28 -preset slow -threads 1 -pass 2 -c:a libopus \
