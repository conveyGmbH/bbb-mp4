...
nice -n 15 ffmpeg -y \
       ...
       -c:v libvpx -vf 'scale=800:-1' -crf 28 -preset slow -threads 0 -pass 1 -an -f null /dev/null
echo "encoding $1-800.webm started pass 2"
nice -n 15 ffmpeg -y \
       -i /usr/src/app/download/$1.mp4 \
       -c:v libvpx -vf 'scale=800:-1' -crf 28 -preset slow -threads 0 -pass 2 -c:a libopus \
